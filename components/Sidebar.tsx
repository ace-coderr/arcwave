...
const NAV = [
  {
    ...
    icon: <svg viewBox="0 0 18 18" fill="currentColor" width="15" height="15"><rect x="1" y="1" width="7" height="7" rx="1.5"/><rect x="10" y="1" width="7" height="7" rx="1.5"/><rect x="1" y="10" width="7" height="7" rx="1.5"/><rect x="10" y="10" width="7" height="7" rx="1.5"/></svg>,
  },
  {
    label: "Payment Links", href: "/links",
    icon: <svg viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="1.6" width="15" height="15"><path d="M10.5 7.5a3.5 3.5 0 010 4.95l-1.5 1.5a3.5 3.5 0 01-4.95-4.95l.75-.75" strokeLinecap="round"/><path d="M7.5 10.5a3.5 3.5 0 010-4.95l1.5-1.5a3.5 3.5 0 014.95 4.95l-.75.75" strokeLinecap="round"/></svg>,
  },
  {
    label: "Transactions", href: "/transactions",
    icon: <svg viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="1.6" width="15" height="15"><path d="M3 6h12M3 6l3-3M3 6l3 3M15 12H3m12 0l-3-3m3 3l-3 3" strokeLinecap="round" strokeLinejoin="round"/></svg>,
  },
  {
    label: "Analytics", href: "/analytics", soon: true,
    icon: <svg viewBox="0 0 18 18" fill="currentColor" width="15" height="15"><path d="M2 14V9h3v5H2zm5 0V5h3v9H7zm5 0V1h3v13h-3z" opacity="0.85"/></svg>,
  },
];
...
export function Sidebar() {
    ...
    <div className="sidebar-inner">
      ...
      {mounted && (
        <div className="sidebar-wallet">
          {isConnected ? (
            <>
              <div className="sidebar-wallet-label">CONNECTED WALLET</div>
              <div className="sidebar-wallet-address">{short}</div>
              <button className="sidebar-disconnect-btn" onClick={() => disconnect()}>Disconnect</button>
            </>
          ) : (
            <>
              <div className="sidebar-wallet-subtitle">Connect wallet to start</div>
      ...
      <nav className="sidebar-nav">
        {NAV.map((item) => (
          <Link
            ...
            className={`sidebar-nav-link${pathname === item.href ? " active" : ""}${item.soon ? " disabled" : ""}`}
          >
        ...
        <div className="sidebar-network-card">
          <div className="sidebar-network-icon">
            <svg viewBox="0 0 14 14" fill="none" width="10" height="10">
              <circle cx="7" cy="7" r="5.5" stroke="var(--brand)" strokeWidth="1.2"/>
              <path d="M7 4.5v2.5l1.5 1" stroke="var(--brand)" strokeWidth="1.2" strokeLinecap="round"/>
            </svg>
          </div>
          <div>
  ...
  return (
    <>
      <aside className="sidebar sidebar-desktop">{inner}</aside>
      ...
      <div className="mobile-topbar">
        ...
        <button className="mobile-hamburger" onClick={() => setMobileOpen(!mobileOpen)}>
          {mobileOpen
            ? <svg viewBox="0 0 24 24" fill="none" width="20" height="20"><path d="M6 6l12 12M6 18L18 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
      ...
      <aside className={`sidebar sidebar-mobile${mobileOpen ? " open" : ""}`}>{inner}</aside>
    </>
  );
}
