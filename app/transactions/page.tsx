...
export default function TransactionsPage() {
  ...
  const refunded = links.filter(l => l.isRefunded);
  const allTx = [...completed, ...refunded].sort((a, b) =>
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ...
      ...allTx.map(tx => [
        tx.title,
        tx.isRefunded ? "Refunded" : tx.isEscrow ? "Escrow" : "Payment",
        tx.amount,
        tx.paidBy ?? "",
      ...
      <div className="page-wrap">
        ...
        {mounted && isConnected && (
          <div className="tx-stats">
            ...
            {refundCount > 0 && (
              <div className="tx-stat">
                <div className="tx-stat-bar" style={{ background: "var(--danger)" }} />
                <div className="tx-stat-val">{refundCount}</div>
                <div className="tx-stat-label">Refunded</div>
              </div>
            )}
          </div>
        )}
        ...
        <div className="card" style={{ display: "flex", flexDirection: "column" }}>
          <div className="table-head">
            <div className="table-head-left">
              ...
              {!isLoading && <span className="count-tag">{allTx.length} records</span>}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button className="btn-sm" onClick={exportCSV} disabled={allTx.length === 0 || isLoading}>
                Export CSV
              </button>
            </div>
          </div>

          {(allTx.length > 0 || isLoading) && (
          ...
          <div style={{ overflowY: "auto", maxHeight: 520 }}>
            ...
            {mounted && isConnected && !isLoading && allTx.length === 0 && (
              <div className="empty">
                <div className="empty-icon">
                  <svg viewBox="0 0 24 24" fill="none" width="22" height="22">
            ...
            {mounted && !isLoading && allTx.map(tx => (
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    ...
                    {tx.isRefunded && (
                      <span style={{
                        ...
                        color: "var(--danger)", background: "rgba(240,62,95,.1)",
                        border: "1px solid rgba(240,62,95,.25)", borderRadius: 4,
                        padding: "1px 5px", letterSpacing: ".06em", flexShrink: 0,
                      }}>REFUNDED</span>
                    )}
                    {tx.isEscrow && !tx.isRefunded && (
                      <span style={{
                        fontSize: 9, fontFamily: "IBM Plex Mono, monospace", fontWeight: 700,
                        color: "#5b8ff9", background: "rgba(91,143,249,.12)",
                        border: "1px solid rgba(91,143,249,.25)", borderRadius: 4,
                        padding: "1px 5px", letterSpacing: ".06em", flexShrink: 0,
                      }}>ESCROW</span>
                    )}
                  </div>
                  {tx.paidAt && (
                    <p className="tx-date-sm">
                      {tx.isRefunded ? "Refunded" : tx.isEscrow ? "Released" : "Paid"} {formatDate(tx.paidAt)}
                    </p>
                  )}
                </div>
