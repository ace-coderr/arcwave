...
export default async function EscrowBuyerPage({ params }: PageProps) {
  ...
  if (escrow.status === "HOLDING" && escrow.releaseDeadline && new Date(escrow.releaseDeadline) <= new Date()) {
    await db.escrowLink.update({ where: { id: escrow.id }, data: { status: "CONFIRMED", confirmedAt: new Date() } });
  ...
  return (
    <EscrowPayPage escrow={{
      id: escrow.id, title: escrow.title, description: escrow.description ?? undefined,
      amount: escrow.amount, sellerAddress: escrow.sellerAddress,
      sellerContact: escrow.sellerContact ?? undefined, buyerAddress: escrow.buyerAddress ?? undefined,
      stealthAddress: escrow.stealthAddress, deliveryDays: escrow.deliveryDays ?? undefined,
      deliveryDeadline: escrow.deliveryDeadline?.toISOString() ?? undefined,
      status: escrow.status, txHash: escrow.txHash ?? undefined,
      ...
      disputeDeadline: escrow.disputeDeadline?.toISOString() ?? undefined,
      sellerRespondedAt: escrow.sellerRespondedAt?.toISOString() ?? undefined,
      buyerLastMessageAt: escrow.buyerLastMessageAt?.toISOString() ?? undefined,
    }}/>
  );
}
