...
const MilestoneIcons: Record<string, JSX.Element> = {
  ...
  earn_1000: <svg viewBox="0 0 24 24" fill="none" width="22" height="22"><path d="M12 2l2.4 4.8 5.6.8-4 3.9.9 5.5L12 14.5l-4.9 2.5.9-5.5L4 7.6l5.6-.8L12 2z" stroke="#f5a623" strokeWidth="1.5" strokeLinejoin="round"/><path d="M12 6l1.5 3 3.5.5-2.5 2.4.6 3.4L12 13.8l-3.1 1.5.6-3.4L7 9.5l3.5-.5L12 6z" fill="#f5a623" opacity=".2"/></svg>,
...
export default function AnalyticsPage() {
  ...
  useEffect(() => { if (isConnected && address) fetchLinks(); }, [isConnected, address, fetchLinks]);
  ...
  completed.forEach(l => { dayCount[new Date(l.paidAt ?? l.createdAt).getDay()]++; });
  const busyDayIdx = dayCount.indexOf(Math.max(...dayCount));
  const DAY_NAMES = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const busyDay = completed.length > 0 ? DAY_NAMES[busyDayIdx] : "—";

  const avgHours = (() => {
    const withPaid = completed.filter(l => l.paidAt);
  ...
  const rangeCompleted = range === "all" ? completed : completed.filter(l => new Date(l.paidAt ?? l.createdAt) >= rangeStart);
  const rangeEarned = rangeCompleted.reduce((s, l) => s + parseFloat(l.amount), 0);
  const maxAmount = Math.max(...chartData.map(d => d.amount), 1);

  const earnedMilestones = MILESTONES.filter(m => m.req(completed.length, totalEarned, stealthCompleted, completionRate, links.length));
  const lockedMilestones = MILESTONES.filter(m => !m.req(completed.length, totalEarned, stealthCompleted, completionRate, links.length));
  ...
  const streak = (() => {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      if (completed.some(l => (l.paidAt ?? l.createdAt)?.startsWith(ds))) s++;
      else if (i > 0) break;
    }
    return s;
  })();
  ...
  const recent = [...completed].sort((a, b) => new Date(b.paidAt ?? b.createdAt).getTime() - new Date(a.paidAt ?? a.createdAt).getTime()).slice(0, 8);
  ...
  const profileUrl = address ? `${typeof window !== "undefined" ? window.location.origin : ""}/u/${address}` : "";
      ...
      <div className="page-wrap">
        ...
        <div className="page-header" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          ...
          {mounted && isConnected && address && (
            <button onClick={copyProfile} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", background: copied ? "var(--c-dim)" : "var(--surface)", border: `1px solid ${copied ? "var(--c-border)" : "var(--stroke)"}`, borderRadius: "var(--r-md)", cursor: "pointer", fontSize: 12, fontWeight: 700, color: copied ? "var(--c)" : "var(--ink-2)", fontFamily: "Sora, sans-serif", transition: "all .2s" }}>
              <svg viewBox="0 0 16 16" fill="none" width="12" height="12">
                <path d="M10 2H4a2 2 0 00-2 2v8a2 2 0 002 2h6a2 2 0 002-2V4a2 2 0 00-2-2z" stroke="currentColor" strokeWidth="1.3"/>
                <path d="M12 4h.5A1.5 1.5 0 0114 5.5v7a1.5 1.5 0 01-1.5 1.5H7A1.5 1.5 0 015.5 12.5V12" stroke="currentColor" strokeWidth="1.3"/>
              </svg>
        ...
        {mounted && isConnected && !isLoading && (
          <>
            ...
            <div className="stats-grid" style={{ marginBottom: 24 }}>
              {[
                { label: "Total Earned", value: fmt(totalEarned), unit: "USDC", sub: `${completed.length} payments`, color: "var(--c)" },
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
              {[
                {
                  label: "Most Active Day", value: busyDay,
                  icon: <svg viewBox="0 0 20 20" fill="none" width="20" height="20"><rect x="3" y="4" width="14" height="13" rx="2" stroke="var(--c)" strokeWidth="1.3"/><path d="M7 2v4M13 2v4M3 9h14" stroke="var(--c)" strokeWidth="1.3" strokeLinecap="round"/></svg>
                },
                {
                  label: "Avg Time to Pay", value: avgHours !== null ? (avgHours < 1 ? `${Math.round(avgHours * 60)}m` : `${Math.round(avgHours)}h`) : "—",
                  icon: <svg viewBox="0 0 20 20" fill="none" width="20" height="20"><circle cx="10" cy="11" r="7" stroke="var(--c)" strokeWidth="1.3"/><path d="M10 8v3l2 1.5" stroke="var(--c)" strokeWidth="1.3" strokeLinecap="round"/><path d="M7 2h6" stroke="var(--c)" strokeWidth="1.3" strokeLinecap="round"/></svg>
                },
                {
                  label: "Stealth Usage", value: `${stealthPct}%`,
                  icon: <svg viewBox="0 0 20 20" fill="none" width="20" height="20"><rect x="4" y="9" width="12" height="9" rx="2" stroke="var(--c)" strokeWidth="1.3"/><path d="M7 9V7a3 3 0 016 0v2" stroke="var(--c)" strokeWidth="1.3" strokeLinecap="round"/></svg>
                },
              ].map((s) => (
                  ...
                  <>
                    ...
                    <div style={{ marginTop: 16, paddingTop: 12, borderTop: "1px solid var(--stroke)", display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 11, color: "var(--ink-3)", fontFamily: "IBM Plex Mono, monospace" }}>Daily volume</span>
                      </div>
                      <div style={{ fontSize: 12, color: "var(--ink-3)" }}>Peak day: <span style={{ color: "var(--c)", fontWeight: 700, fontFamily: "IBM Plex Mono, monospace" }}>{fmt(maxAmount)} USDC</span></div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
              <div className="card">
                ...
                <div style={{ padding: "0 0 8px" }}>
                  {topLinks.length === 0 ? (
                    <div style={{ padding: "24px", textAlign: "center", color: "var(--ink-3)", fontSize: 12 }}>No completed payments yet</div>
                  ) : topLinks.map((l, i) => (
                    <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px", borderBottom: i < topLinks.length - 1 ? "1px solid var(--stroke)" : "none" }}>
                      ...
                      <span style={{ fontSize: 13, fontFamily: "IBM Plex Mono, monospace", fontWeight: 700, color: "var(--c)", flexShrink: 0 }}>{fmt(parseFloat(l.amount))} <span style={{ fontSize: 10, color: "var(--ink-3)" }}>USDC</span></span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div className="card" style={{ flex: 1 }}>
                  ...
                  <div style={{ padding: "0 0 8px", maxHeight: 220, overflowY: "auto" }}>
                    {recent.length === 0 ? (
                      <div style={{ padding: "24px", textAlign: "center", color: "var(--ink-3)", fontSize: 12 }}>No payments yet</div>
                    ) : recent.map((l, i) => (
                      <div key={l.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", borderBottom: i < recent.length - 1 ? "1px solid var(--stroke)" : "none" }}>
                        ...
                        <span style={{ fontSize: 12, fontFamily: "IBM Plex Mono, monospace", fontWeight: 700, color: "var(--c)", flexShrink: 0, marginLeft: 12 }}>+{fmt(parseFloat(l.amount))}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* Share CTA */}
            <div style={{ marginTop: 18, background: "linear-gradient(135deg, rgba(0,229,160,.08), rgba(91,143,249,.06))", border: "1px solid var(--c-border)", borderRadius: "var(--r-xl)", padding: "24px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
              <div>
                <p style={{ fontSize: 15, fontWeight: 800, color: "var(--ink-1)", marginBottom: 4, letterSpacing: "-.02em" }}>Share your payment profile</p>
                <p style={{ fontSize: 12, color: "var(--ink-3)" }}>Let others pay you — or show off your stats</p>
                {address && <p style={{ fontSize: 11, color: "var(--ink-3)", fontFamily: "IBM Plex Mono, monospace", marginTop: 6 }}>/u/{address.slice(0, 10)}...{address.slice(-4)}</p>}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                {address && (
                  <Link href={`/u/${address}`} target="_blank" style={{ padding: "10px 18px", background: "var(--surface)", border: "1px solid var(--stroke)", borderRadius: "var(--r-md)", fontSize: 13, fontWeight: 700, color: "var(--ink-1)", textDecoration: "none" }}>
                    Preview ↗
                  </Link>
                )}
                <button onClick={copyProfile} style={{ padding: "10px 18px", background: "var(--c)", border: "none", borderRadius: "var(--r-md)", fontSize: 13, fontWeight: 700, color: "#000", cursor: "pointer", fontFamily: "Sora, sans-serif" }}>
                  {copied ? "Copied!" : "Copy Link"}
                </button>
              </div>
            </div>

            <PnlCard address={address!} totalEarned={totalEarned} completedCount={completed.length} completionRate={completionRate} avgPayment={avgPayment} biggestPayment={biggestPayment} />
          </>
        )}
      </div>
